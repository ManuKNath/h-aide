...
from .forms import loginForm,registerForm,add_details,applicationForm,schemeForm,edit_details
...
def user_login(request):
	...
	if myform.is_valid():
		...
		if not user==None:
			...
			print("Login Failed")
			return render(request,"user_login.html",{"form":myform,"invalid":True})
	...
	if myform.is_valid():
		...
		if not user==None:
			...
			if request.user.is_superuser:
				...
				return render(request,"user_login.html",{"form":myform,"adlogerr":True})	
		else:
			print("Login Failed")
			return render(request,"admin_login.html",{"form":myform,"invalid":True})
...
def editdetails(request):
	detailform = edit_details(request.POST or None)
	context ={"form" : detailform}		
	if detailform.is_valid():
		first_name = detailform.cleaned_data.get('first_name')
		last_name = detailform.cleaned_data.get('last_name')
		...
		job = detailform.cleaned_data.get('job')
		income = detailform.cleaned_data.get('income')
		user_id = detailform.cleaned_data.get('user_id')
		Details.objects.filter(user_id=user_id).update(fname=first_name,lname=last_name,address=address,age=age,aadhar_id=aadhar_id,disability=disablity,category=category,percentage_disabliity=percentage_disabliity,ward_number=ward,house_number=house,village=village,educational_qualification=education,job=job,income=income)
		return redirect(home_page)
